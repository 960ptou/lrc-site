    ...
    const fetchData = async () => {
        try {
            ...
            for (let identifier of resources.data) {
                table[identifier] = (
                    await axios.get(`/api/resource/${identifier}`)
                ).data;
            }
    ...
    return (
        <div>
            <Grid container spacing={2} sx={{ marginLeft: 2 }}>
                {resourceTable &&
                    Object.entries(resourceTable).map(([identifier, items]) => (
                        <Grid
                            item="true"
                            xs={12}
                            sm={6}
                            md={4}
                            sx={{ width: 128, height: 196 }}
                            key={identifier}
                        >
                            <ImageCard
                                imageSources={[
            ...
            {sideBarOpen && sideBarSelection && sideBar && (
                <ImageDrawer
                    ...
                    clickWrapper={(ind) => {
                        ...
                        setSubtitleLk(
                            `/api/subtitle/${sideBarSelection}/${ind}`
                        );
            ...
            {audioLk && subtitleLk && (
                <LyricsAudioPlayer
                    ...
                    onAudioEnd={() => {
                        ...
                        const currentIndx = Number.parseInt(
                            audioLk
                                .match(/\/[^\/]+\/[^\/]+\/(\d+)/)[1]
                                .replace("/", "")
                        );
